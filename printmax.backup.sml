...
fun showGateWithForwardDeclarations ((device, input, output), (inDefs, inStr, inForwardDefs)) =
	...
	let val inNodes = exprNodes input
	    val outNodes = exprNodes output
		(* build string of any forward definitions *)
		val (_, forwardDefsStr, forwardDefs2) = 
			List.foldl forwardDeclareIfUndefined (inDefs, "", inForwardDefs) inNodes
	in
		case outNodes of
			[] => (
		  ...
		  | [outp] => (
		   ...
		   | _ => (
				Errors.simple_error "Maxj: all devices must have one output (n)"
		)
	end
