(* tjt: this is catted together from files in the SML 110 distribution,
        following example of 'base.sml' in old Ruby compiler.
		Files copied from $SMLNJ_INSTALL/ml-yacc/lib/
		where SMLNJ_INSTALL is base of SML/NJ installation

		Note had to change:
		   stdout => TextIO.std_out
		   output => TextIO.output
		   makestring -> Int.toString
*)
...
structure LrParser :> LR_PARSER =
 struct
   (* tjt: output => TextIO.output, std_out => TextIO.stdOut *)
	...
	struct
     ...
     exception ParseError
	  ...
	  (*tjt*)
	  val makestring = fn (i) => Int.toString(i)
