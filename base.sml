...
(* tjt97: replace abstraction with opaque signature
  * abstraction no longer supported in SML 97
  * see: http://www.smlnj.org/doc/Conversion/modules.html#SigDefs
  * abstraction S : SIG = struct ... end  =>  structure S :> SIG = struct ... end
  * old first line of this decl was:
...
structure LrParser :> LR_PARSER =
   struct
      ...
      (* tjt97: more replace abstraction with opaque structure; see line 307
	  ...
	  *)
      ...
      local 
         ...
         val showState = fn (STATE s) => "STATE " ^ (Int.toString s) (* tjt97 *)
      in
        fun printStack(stack: ('a,'b) stack, n: int) =
         case stack
           of (state,_) :: rest =>
                 (print("\t" ^ Int.toString n ^ ": ");  (* tjt97 *)
             ...
             (println "Parse: state stack:";
              ...
              case action
                of SHIFT state => println ("SHIFT " ^ (showState state))
                 | REDUCE i => println ("REDUCE " ^ (Int.toString i)) (*tjt97*)
                 | ERROR => println "ERROR"
...
val mkFixError = fn ({is_keyword,preferred_subst,terms,errtermvalue,
  ...
  let fun FixError(lexv as (TOKEN (term,value as (_,leftPos,_)),_),
    ...
    let val lexVList = map (fn t => TOKEN (t,(errtermvalue t,leftPos,leftPos)))
		...
		(* tjt97 *)
        val (_,numStateList) = List.foldr (fn (a,(num,r)) => (num+1,(a,num)::r))
                                (0,nil) stateList
        ...
        val operToString = 
         ...
         val printChange = fn c =>
          ...
          in (print ("{distance= " ^ (Int.toString distance));  (* tjt97 *)
              print (",orig = " ^ (showTerminal t'));
              print (",new = " ^ (showTerminal t));
              print (",oper= " ^ (operToString oper));
              print (",pos= " ^ (Int.toString pos));  (* tjt97 *)
              print "}\n")
          end
...
(* foldStateList: accumulates results while scanning numStateList *)
        ...
        fun foldStateList f start = List.foldr f start numStateList (* tjt97 *)

(* foldLexVList: accumulates results while scanning lexVList *)

        fun foldLexVList f start = List.foldr f start lexVList (* tjt97 *)
...
(* substFold: accumulate results of deleting the current terminal
   and then trying to insert tokens *)
        ...
        val findMaxDist = fn l => 
          foldr (fn (CHANGE {distance,...},high) => Int.max(distance,high)) 0 l (* tjt97 *)
...
(* sieve: keep only the elements of a list for which pred is true *)
        ...
        val sieve = fn pred => fn l => 
          foldr (fn (elem,rest) => if pred elem then elem::rest else rest) nil l  (* tjt97 *)
...
(* Remove keywords which don't meet the long parse check
                    ...
                    *)
                    ...
                    val restQueue = 
                     Fifo.put((stack,newLexPair),
                              foldl Fifo.put Fifo.empty queueFront) (* tjt97 *)
   ...
   val parse = fn {arg,table,lexer,saction,void,lookahead,
        ...
        let val distance = 15   (* defer distance tokens *)
            ...
            val maxAdvance = Int.max(lookahead,0)(* max distance for parse check *)  (* tjt97 *)
